/**
 * POST /api/payroll/revisions/[id]/release-hold — HOLD → SUBMITTED.
 */
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (record.status !== 'HOLD') {
    return NextResponse.json({ error: `Cannot release hold on a revision in ${record.status} status.` }, { status: 409 });
  }

  const updated = await prisma.salaryRevisionRequest.update({
    where: { id: record.id },
    data: { status: 'SUBMITTED', holdReason: null },
  });
