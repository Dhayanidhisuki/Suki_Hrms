...
export async function PUT(
  ...
  const permCheck = await checkModulePermission(authCheck.session, "tool_group", "EDIT");
...
export async function DELETE(
  ...
  const permCheck = await checkModulePermission(authCheck.session, "tool_group", "DELETE");
  if (!permCheck.allowed) return NextResponse.json({ success: false, error: "Forbidden" }, { status: 403 });
