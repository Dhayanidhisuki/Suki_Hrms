/**
 * POST /api/gratuity/records/[id]/reject — CALCULATED → REJECTED. Requires
 * rejectReason.
 */
...
import { rejectGratuitySchema } from '@/lib/validations/gratuity';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = rejectGratuitySchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
  ...
  if (record.status !== 'CALCULATED') {
    return NextResponse.json({ error: `Cannot reject a gratuity record in ${record.status} status.` }, { status: 409 });
  }
