/**
 * POST /api/gratuity/records/[id]/release-hold — HOLD → CALCULATED.
 */
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (record.status !== 'HOLD') {
    return NextResponse.json({ error: `Cannot release hold on a gratuity record in ${record.status} status.` }, { status: 409 });
  }
