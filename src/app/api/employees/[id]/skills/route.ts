/**
 * GET  /api/employees/[id]/skills   — list skill matrix records
 * POST /api/employees/[id]/skills   — add a skill record
 */
...
export async function POST(
  ...
  const record = await prisma.$transaction(async (tx) => {
    const created = await tx.employeeSkill.create({ data: { employeeId, ...parsed.data } });
