...
export async function PUT(
  ...
  const permCheck = await checkModulePermission(authCheck.session, "tool_issue_receive", "EDIT");
...
export async function DELETE(
  ...
  const permCheck = await checkModulePermission(authCheck.session, "tool_issue_receive", "DELETE");
  if (!permCheck.allowed) return NextResponse.json({ success: false, error: "Forbidden" }, { status: 403 });
