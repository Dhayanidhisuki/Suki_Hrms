...
const COLUMNS = [
  { key: "name", label: "Group", mono: false },
  { key: "prefixToolsNo", label: "Tools Prefix", mono: true },
  { key: "itemNoPrefixMod", label: "Prefix Mod", mono: false },
  { key: "prefixGateEntry", label: "Gate Entry", mono: true }
] as const;
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await apiGet<{ items?: Row[] }>("/api/lookups/groups");
      if (!cancelled) {
  ...
  return (
    <SimpleMasterShell title="Tool Numbering" subtitle="Prefix fields from Tool Group master (OTHER_TOOLS_TYPE)">
