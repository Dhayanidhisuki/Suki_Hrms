...
import { Download, Trash2, Upload, FileText, X, Eye, Camera } from "lucide-react";
    ...
    !loading && items.length > 0 ? (
      <ul className="rounded-[12px] border-[0.5px] border-[var(--border-main)] bg-[var(--bg-card)] divide-y divide-[var(--border-main)] overflow-hidden">
        {items.map((d) => (
          ...
          >
            <div className="min-w-0 flex-1 cursor-pointer" onClick={() => setPreviewDoc(d)}>
              <p
                className="text-xs font-semibold text-[var(--text-primary)] hover:text-[var(--primary)] truncate"
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <span className="inline-flex h-5 items-center rounded-md border-[0.5px] border-[var(--border-main)] bg-[var(--bg-subtle)] px-1.5 text-[10px] font-semibold text-[var(--text-secondary)]">
                  {DOC_TYPE_LABELS[d.docType as ToolDocType] ?? d.docType}
                </span>
            ...
            <div className="flex shrink-0 items-center gap-0.5">
              <button
                ...
                className="inline-flex p-1.5 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)]"
              >
          ...
          <tbody className="divide-y divide-[var(--border-main)]">
            {items.map((d) => (
              <tr key={d.id} className="hover:bg-[var(--bg-hover)]">
                <td className="py-2 px-2 max-w-[160px] cursor-pointer" onClick={() => setPreviewDoc(d)}>
                  <p
                    className="font-semibold text-[var(--text-primary)] hover:text-[var(--primary)] truncate"
                    title={d.originalName}
                  >
                    {d.originalName}
                  </p>
                ...
                <td className="py-2 px-2 text-right whitespace-nowrap">
                  <button
                    ...
                    title="Preview"
                    onClick={() => setPreviewDoc(d)}
                    className="inline-flex p-1.5 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] mr-0.5"
                  >
                    <Eye className="w-3.5 h-3.5" />
                  </button>
                  <button
          ...
          {canUpload && (
            <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_auto_auto] gap-2 items-end">
                ...
                >
                  {types.map((t) => (
                    <option key={t} value={t}>
                      {DOC_TYPE_LABELS[t as ToolDocType] ?? t}
                    </option>
                  ))}
                </select>
              </label>
              ...
              <Button
                ...
                className="h-9 px-2.5 rounded-[12px] border-[0.5px]"
              >
